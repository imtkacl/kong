...
local ldap = require "kong.plugins.ldap-auth-plus.ldap"
...
local function ldap_authenticate(given_username, given_password, conf)
  ...
  if not ok then
    ngx_log(ngx_error, "[ldap-auth-plus] failed to connect to "..conf.ldap_host..":"..tostring(conf.ldap_port)..": ", err)
  ...
  if not ok then
    ngx_log(ngx_error, "[ldap-auth-plus] failed to keepalive to "..conf.ldap_host..":"..tostring(conf.ldap_port)..": ", suppressed_err)
  end
  return is_authenticated, err
end

local function load_credential(given_username, given_password, conf)
  ngx_log(ngx_debug, "[ldap-auth-plus] authenticating user against LDAP server: "..conf.ldap_host..":"..conf.ldap_port)

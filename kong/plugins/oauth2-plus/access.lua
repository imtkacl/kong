...
local function generate_token(conf, api, credential, authenticated_userid, scope, state, expiration, disable_refresh)
  ...
  local token, err = singletons.dao.oauth2p_tokens:insert({
    api_id = api_id,
...
local function load_oauth2_credential_by_client_id_into_memory(client_id)
  local credentials, err = singletons.dao.oauth2p_credentials:find_all {client_id = client_id}
  if err then
    return nil, err
  end
...
local function get_redirect_uri(client_id)
  local client, err
  if client_id then
    client, err = cache.get_or_set(cache.oauth2p_credential_key(client_id), nil,
    ...
    elseif not parameters.authenticated_userid or utils.strip(parameters.authenticated_userid) == "" then
      ...
      if not response_params[ERROR] then
        if response_type == CODE then
          ...
          local authorization_code, err = singletons.dao.oauth2p_authorization_codes:insert({
            api_id = api_id,
  ...
  if not is_https then
    ...
    if not response_params[ERROR] then
      if grant_type == GRANT_AUTHORIZATION_CODE then
        ...
        local authorization_code = code and singletons.dao.oauth2p_authorization_codes:find_all({api_id = api_id, code = code})[1]
        ...
        elseif authorization_code.credential_id ~= client.id then
          ...
          singletons.dao.oauth2p_authorization_codes:delete({id=authorization_code.id}) -- Delete authorization code so it cannot be reused
        end
      ...
      elseif grant_type == GRANT_REFRESH_TOKEN then
        ...
        local token = refresh_token and singletons.dao.oauth2p_tokens:find_all({api_id = api_id, refresh_token = refresh_token})[1]
        if not token then
          ...
          singletons.dao.oauth2p_tokens:delete({id=token.id}) -- Delete old token
        end
      end
    end
  end
...
local function load_token_into_memory(conf, api, access_token)
  ...
  local credentials, err = singletons.dao.oauth2p_tokens:find_all { api_id = api_id, access_token = access_token }
  local result
  if err then
    return nil, err
...
local function retrieve_token(conf, access_token)
  ...
  if access_token then
    token, err = cache.get_or_set(cache.oauth2p_token_key(access_token), nil,
...
local function load_oauth2_credential_into_memory(credential_id)
  local result, err = singletons.dao.oauth2p_credentials:find {id = credential_id}
  if err then
    return nil, err
  end
  return result
...
local function do_authentication(conf)
  ...
  local credential, err = cache.get_or_set(cache.oauth2p_credential_key(token.credential_id),
